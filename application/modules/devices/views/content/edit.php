	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
			...
			<div class="control-group <?php echo form_error('bootloader_exploit') ? 'error' : ''; ?>">
				...
				<div class='controls' aria-labelled-by='devices_bootloader_exploit_label'>
					<label class='radio' for='devices_bootloader_exploit_option1'>
						<input id='devices_bootloader_exploit_option1' name='devices_bootloader_exploit' type='radio' class='' value='0' <?php echo set_radio('devices_bootloader_exploit',  0, $devices['bootloader_exploit'] == 0 ? TRUE : FALSE); ?> />
						Mako Exploit
					</label>
					<label class='radio' for='devices_bootloader_exploit_option2'>
						<input id='devices_bootloader_exploit_option2' name='devices_bootloader_exploit' type='radio' class='' value='1' <?php echo set_radio('devices_bootloader_exploit',  1, $devices['bootloader_exploit'] == 1 ? TRUE : FALSE); ?> />
						Loki
					</label>
					<label class='radio' for='devices_bootloader_exploit_option2'>
						<input id='devices_bootloader_exploit_option3' name='devices_bootloader_exploit' type='radio' class='' value='2' <?php echo set_radio('devices_bootloader_exploit',  2, $devices['bootloader_exploit'] == 2 ? TRUE : FALSE); ?> />
						Classified Exploit
					</label>
			...
			<div class="control-group <?php echo form_error('maintainers') ? 'error' : ''; ?>">
				...
				<div class='controls'>
					<?php $this->load->model('users/user_model'); echo $this->user_model->createOptions('devices_maintainers[]', $this->user_model->find_all_names(), explode(",", $devices['maintainers']), 'checkbox'); ?>
				</div>
			</div>
			...
			<div class="control-group <?php echo form_error('actions') ? 'error' : ''; ?>">
				...
				<div class='controls'>
					...
					$preSetArray = array();
					if($devices['actions'] == 1){
						$actions = $this->devices_model->get_actions_by_model($devices['model']);
						if(is_array($actions)){
							foreach($actions as $action_id => $action){
								$array = get_object_vars($action);
								array_push($preSetArray, $array['action_name']);
							}
						}
					}
					echo $this->actions_model->createOptions('devices_actions[]', $this->actions_model->find_all_names(), $preSetArray, 'checkbox'); ?>
				</div>
			</div>
			...
			<div class="form-actions">
				<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('devices_action_edit'); ?>"  />
				<?php echo lang('bf_or'); ?>
				<?php echo anchor(SITE_AREA .'/content/devices', lang('devices_cancel'), 'class="btn btn-warning"'); ?>
				
			<?php if ($this->auth->has_permission('Devices.Content.Delete')) : ?>
				or
				<button type="submit" name="delete" class="btn btn-danger" id="delete-me" onclick="return confirm('<?php e(js_escape(lang('devices_delete_confirm'))); ?>'); ">
					<span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('devices_delete_record'); ?>
				</button>
			<?php endif; ?>
			</div>
		</fieldset>
